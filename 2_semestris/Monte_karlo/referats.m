...
%% konverģences grafiks
N = 10000;
estimates_conv = zeros(N, 2);
...
for n = 1:N
    ...
    est_area = 14 * 4 * h / n;
    est_area_cv = 14 * 3 * h_c / n - 14 * 1.5 + exact_area_Y;

    estimates_conv(n, 1) = abs(est_area - exact_area);
    estimates_conv(n, 2) = abs(est_area_cv - exact_area);
end;
plot(estimates_conv(:, 1))
hold on
plot(estimates_conv(:, 2))
legend('Bez kontroles', 'Ar kontroli')
title('Konverģence')
hold off